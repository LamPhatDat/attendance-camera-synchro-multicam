...
class ReIDGallery:
    ...
    def link_in_roi(self, gid: int, name: str) -> bool:
        with self.lock:
            ...
            return True
    def force_link(self, gid: int, name: str) -> bool:
        """Gán tên bằng khuôn mặt — KHÔNG cần pending, KHÔNG cần ROI.
        Dùng khi face nhận diện chắc chắn (quyền lực tối cao)."""
        with self.lock:
            # Nếu tên này đang gắn ở gid khác → dời sang gid mới
            old_gid = self._name_to_global_id.get(name)
            if old_gid == gid:
                return True  # đã đúng rồi, khỏi làm lại
            if old_gid is not None:
                self._linked_targets.pop(old_gid, None)
            # Nếu gid này đang mang tên khác → gỡ tên cũ ra
            old_name = self._linked_targets.get(gid)
            if old_name is not None and old_name != name:
                self._name_to_global_id.pop(old_name, None)
            self._linked_targets[gid] = name
            self._name_to_global_id[name] = gid
            logger.info("FACE LINKED: '%s' → global_id=%d (bằng khuôn mặt)", name, gid)
            return True
